...
const cacheKeySig = (hash, keyB64) => `sig|${hash}|${keyB64}`;
const importPublicKeyForVerify = async (publicKeyB64, webHash) => {
    const k = cacheKeySig(webHash, publicKeyB64);
    const cached = pkCache().get(k);
    if (cached !== undefined)
        return cached;
    const key = await subtle.importKey("spki", fromB64(publicKeyB64), { name: "RSASSA-PKCS1-v1_5", hash: webHash }, false, ["verify"]);
    pkCache().set(k, key);
    return key;
};
const importPrivateKeyForSign = async (privateKeyB64, webHash) => {
    const k = cacheKeySig(webHash, privateKeyB64);
    const cached = pvCache().get(k);
    if (cached !== undefined)
        return cached;
    const key = await subtle.importKey("pkcs8", fromB64(privateKeyB64), { name: "RSASSA-PKCS1-v1_5", hash: webHash }, false, ["sign"]);
    pvCache().set(k, key);
    return key;
};
...
export const CryptoUtil = {
    ...
    sign: async (privateKey, data, parameters) => {
        const { webHash } = resolveParams(parameters);
        const privB64 = resolvePrivateKeyB64(privateKey);
        const keyObj = await importPrivateKeyForSign(privB64, webHash);
        const bytes = new TextEncoder().encode(data);
        const sigBuf = await subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, keyObj, bytes);
        return toB64(sigBuf);
    },
    verifySignature: async (publicKey, data, signatureB64, parameters) => {
        const { webHash } = resolveParams(parameters);
        const pubB64 = resolvePublicKeyB64(publicKey);
        const keyObj = await importPublicKeyForVerify(pubB64, webHash);
        const bytes = new TextEncoder().encode(data);
        const sigBytes = fromB64(signatureB64);
        const ok = await subtle.verify({ name: "RSASSA-PKCS1-v1_5" }, keyObj, sigBytes, bytes);
        return ok;
    },
};
