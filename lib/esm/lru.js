...
export class LRUCache {
    ...
    get(key) {
        ...
        if (this.logger && this.logger.isTraceEnabled()) {
            this.logger.trace(`LRUCache.get(${key}) now=${this.clock.now()} entry=${entry ? JSON.stringify(entry) : "undefined"}`);
        }
        if (!entry) {
            return undefined;
        }
        if (this.maxAge !== undefined && this.clock.now() - entry.timestamp > this.maxAge) {
            if (this.logger && this.logger.isTraceEnabled()) {
                this.logger.trace(`LRUCache.get(${key}) now=${this.clock.now()} EXPIRED entry=${entry ? JSON.stringify(entry) : "undefined"}`);
            }
        ...
        if (this.touchOnGet) {
            if (this.logger && this.logger.isTraceEnabled()) {
                this.logger.trace(`LRUCache.get(${key}) now=${this.clock.now()} TOUCH-ON-GET entry=${entry ? JSON.stringify(entry) : "undefined"}`);
            }
            this.cache.delete(key);
