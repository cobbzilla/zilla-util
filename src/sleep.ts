...
export const DEFAULT_CLOCK: YuebingClock = {
    now: () => Date.now(),
};
...
const _nap =
    (resolve: (v?: unknown) => void, clock: YuebingClock, alarm: NapAlarm, ms: number, check: number) => async () => {
        try {
...
export const nap = (clock: YuebingClock, alarm: NapAlarm, ms: number, check?: number) =>
